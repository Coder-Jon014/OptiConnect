...
export default function AuthenticatedLayout({ user, header,subheader, children }) {
  const [showingNavigationDropdown, setShowingNavigationDropdown] = useState(false)
//   const { props } = usePage() // Get flash messages

  return (
    <div className="min-h-screen bg-[var(--background)]">
      <nav className="bg-[var(--background)] border-b border-gray-100 dark:border-gray-700">
                ...
                <Dropdown>
                  <Dropdown.Trigger>
                    <span className="inline-flex rounded-md">
                      <button
                        type="button"
                        className="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-black bg-white dark:bg-[var(--background)] hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150"
                      >
                        {user.name}
      ...
      {header && (
        <header className="bg-[var(--background)] shadow">
          <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">{header}{subheader}</div>
        </header>
      )}
